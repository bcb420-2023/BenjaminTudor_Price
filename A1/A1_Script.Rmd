...
raw_expression_data = read.delim("/Users/ben/Desktop/BCB420/RProjects/BCB420/A1/GSE164122/GSE164122_cRPKM_AND_COUNTS-Ath24.txt",header=TRUE,
 check.names = FALSE)
...
code_to_treatment <- c('light', 'dark')
...
get_sample_info <- function(column_name){
  ...
  group <- paste(c(light_condition, sample_info[[2]]), collapse = '_')
...
new_names <- apply(samples, MARGIN=1, FUN=function(x){return(paste(x[1:3], collapse='_'))})

rownames(samples) <- new_names

names(raw_counts)[3:length(raw_counts)] <- new_names
...
normalized_cpms <- edgeR::cpm(normalized_counts)
normalized_cpms <- cbind(raw_counts[1:2], normalized_cpms)
write.csv(samples, "/Users/ben/Desktop/BCB420/RProjects/BCB420/samples.csv", row.names=FALSE)

normalized_counts <- normalized_cpms
...
all_names$`Merge Names` <- normalized_counts[,2]

all_names$`Merge Names`[! is.na(all_names[, 3])] <-  
  unlist(lapply(all_names[, 3][! is.na(all_names[, 3])], 
         function(x){return(unlist(strsplit(x, split=' '))[[1]])}))

# Update normalized counts with new names.
normalized_counts$NAME <- all_names$`Merge Names`
...
genes <- read.csv(file.path(getwd(),"genes.csv"), sep='\t', quote = "")
genes$Gene.Model.Type <- lapply(genes$Gene.Model.Type, function(x){gsub("[\\s ( )]", "", x)})
gene_names <- genes$Gene.Model.Type
names(gene_names) <- row.names(genes)



for(i in seq_along(normalized_counts$NAME)){
  normalized_counts$NAME[i] <- gene_names[normalized_counts$ID[i]]
}
...
write.csv(normalized_counts, "/Users/ben/Desktop/BCB420/RProjects/BCB420/normalized_cpms.csv", row.names=FALSE)
...
So after assignment 1, we now have our normalised dataset (normalised_counts). Which is still formatted as raw reads. We also have the replicate groups (samples). Now, let's calculate the differential between the genes in our dataset.
