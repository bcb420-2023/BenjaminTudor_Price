...
if (!requireNamespace("imager"))
    install.packages("imager", quietly=TRUE)
...
3. These results are in agreement with the previous analyses, but were able to detect enrichment for DE of high energy metabolic procceses far more effectively. In general comparison of these results is fairly stright forward if we assume that the adjusted p-values provide comparable accuracy, however the inclusion of wikipathways in this iteration makes the anlysis slightly more complex.
...
```
 ...
 Figure 1. A screenshot of the original unmodified network, with node significance threshold q < 0.1 and edge threshold q < 0.375. This graph contains 189 nodes and 658 nodes.
2. See below.
3.
```{r}
...
```
 ...
 Figure 4. A heatmap showing the relative expression levels of genes with and adjusted p value < `reg_p_val_th`, but without any annotation among all annotations used for GSEA. Annotations include GO Biological Process, GO Molecular Funciton, and GO Cellular Component (excluding electronic annotaions).
...
```
 ...
 Figure 5. A heatmap showing the relative expression levels of up regulated genes with an adjusted p value < `up_reg_p_val_th` not found to be significantly enriched (p_adj < `go_p_val_thr`) amoung any of the given annotations. Annotations include GO Biological Process, GO Molecular Function, and GO Cellular Component (excluding electronic annotations).
...
Figure 6. A heatmap showing the relative expression levels of down regulated genes with an adjusted p value < `down_reg_p_val_th` not found to be significantly enriched (p_adj < `go_p_val_thr`) among any of the given annotations. Annotations include GO Biological Process, GO Molecular Function, and GO Cellular Component (excluding electronic annotations).
